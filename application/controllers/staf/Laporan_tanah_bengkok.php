<?php
Class Laporan_tanah_bengkok extends CI_Controller{
    ...
    public function index(){
        ...
        $pdf->Cell(3, 0.8, 'Bengkok Sawah', 1, 0, 'C');
        $pdf->Cell(4, 0.8, 'Bengkok Lahan Kering', 1, 0, 'C');
        $pdf->Cell(3.2, 0.8, 'Kas Sawah', 1, 0, 'C');
        $pdf->Cell(3, 0.8, 'Kas Lahan Kering', 1, 0, 'C');
        $pdf->Cell(3, 0.8, 'Tahun', 1, 0, 'C');

        $pdf->SetFont('Arial','',8);


        $dataTanahbengkok = $this->db->get('tb_tanah_bengkok')->result();
        foreach ($dataTanahbengkok as $row){
          ...
          $pdf->Cell(3,0.8,$row->bengkok_sawah,1,0, 'C');
          $pdf->Cell(4,0.8,$row->bengkok_lahan_kering,1,0, 'C');
          $pdf->Cell(3.2,0.8,$row->kas_sawah,1,0, 'C');
          $pdf->Cell(3,0.8,$row->kas_lahan_kering,1,0, 'C');
