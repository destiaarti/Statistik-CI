<?php
Class Laporan_kependudukan_mutasi extends CI_Controller{
    ...
    public function index(){
        ...
        $pdf->ln(1);
        $pdf->SetFont('Arial','B',12);
        $pdf->Cell(18,0.7,"Laporan Pengelolaan Kependudukan Mutasi",0,10,'C');
        ...
        $pdf->Cell(3, 0.8, 'Jenis Mutasi', 1, 0, 'C');
        ...
        $dataWilayah = $this->db->get('tb_mutasi_kependudukan')->result();
        foreach ($dataWilayah as $row){
          ...
          $pdf->Cell(3,0.8,$row->jenis_mutasi,1,0, 'C');
          $pdf->Cell(3,0.8,$row->jenis_kelamin,1,0, 'C');
