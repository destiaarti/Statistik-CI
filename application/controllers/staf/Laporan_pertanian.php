<?php
Class Laporan_pertanian extends CI_Controller{
    ...
    public function index(){
        ...
        $pdf->Cell(18,0.7,"Laporan Pengelolaan Pertanian",0,10,'C');
        ...
        $pdf->Cell(3, 0.8, 'Luas Panen', 1, 0, 'C');
        $pdf->Cell(3, 0.8, 'Rata-rata Produksi', 1, 0, 'C');
        $pdf->Cell(3.2, 0.8, 'Produksi', 1, 0, 'C');

        $pdf->SetFont('Arial','',8);


        $dataPertanian = $this->db->get('tb_panen_produksi')->result();
        foreach ($dataPertanian as $row){
          $pdf->ln();



          $pdf->Cell(1.8,0.8,$row->id_desa,1,0, 'C');
          $pdf->Cell(3,0.8,$row->luas_panen,1,0, 'C');
          $pdf->Cell(3,0.8,$row->rata_rata_produksi,1,0, 'C');
          $pdf->Cell(3.2,0.8,$row->produksi,1,0, 'C');
      }
