<?php
Class Laporan_listrik extends CI_Controller{
    ...
    public function index(){
        ...
        $pdf->ln(1);
        $pdf->SetFont('Arial','B',12);
        $pdf->Cell(18,0.7,"Laporan Pengelolaan Penggunaan PLN PDAM",0,10,'C');
        ...
        $pdf->Cell(3, 0.8, 'Rumah Tangga', 1, 0, 'C');
        $pdf->Cell(3, 0.8, 'Pelanggan PLN', 1, 0, 'C');
        $pdf->Cell(3.2, 0.8, 'Pelanggan PDAM', 1, 0, 'C');

        $pdf->SetFont('Arial','',8);


        $datapln_pdam = $this->db->get('tb_pln_pdam')->result();
        foreach ($datapln_pdam as $row){
          $pdf->ln();



          $pdf->Cell(1.8,0.8,$row->id_desa,1,0, 'C');
          $pdf->Cell(3,0.8,$row->rumah_tangga,1,0, 'C');
          $pdf->Cell(3,0.8,$row->pelanggan_pln,1,0, 'C');
          $pdf->Cell(3.2,0.8,$row->pelanggan_pdam,1,0, 'C');
      }
